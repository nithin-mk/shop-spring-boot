/**
 * End-to-end Playwright test suite for the Kotlin/Spring Boot E-Commerce Shop.
 *
 * Ported verbatim from the original Node.js app's e2e/app.spec.ts (see
 * nodejs-complete-guide, minio branch) — selectors were kept unchanged
 * because the Thymeleaf templates/CSS/JS were migrated to match the
 * original EJS markup class-for-class, id-for-id.
 *
 * Design notes:
...
/** Image fixture (copied from the Node.js app's test/test-product.png) */
...
test.describe('Signup', () => {
  ...
  test('signup shows validation error for invalid email', async ({ page }) => {
    ...
    // Poll instead of taking a single count() snapshot: right after a
    // toHaveURL() match, WebKit can still be mid-paint of the re-rendered
    // page, so a one-shot count can race the DOM update.
    await expect
      .poll(
        async () =>
